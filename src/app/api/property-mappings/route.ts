...
export async function GET() {
  ...
  try {
    const res = await fetch(`${backendUrl}/api/ai/property-mappings`, {
      headers: {
        'X-API-Key': API_KEY,
      },
      cache: 'no-store',
    ...
    return NextResponse.json({ error: (err as Error).message || 'Failed to fetch property mappings' }, { status: 500 });
  }
}
